...
AUTO_BP_UPDATER = (os.environ['AUTO_BP_UPDATER'] == 'True')

# Create milestone automatically on/off
AUTO_MILESTONE_CREATOR = (os.environ['AUTO_MILESTONE_CREATOR'] == 'True')
...
def lambda_handler(event, context):
    try:
        if JIRA_INTEGRATION and 'Records' in event:
            for record in event['Records']:
                ...
                if len(ddb_query_response) > 0 and ddb_bp_count == 1:
                    ...
                    # Only Update BP in WA Tool and Create Milestone automatically if AUTO_BP_UPDATER and AUTO_MILESTONE_CREATOR are true
                    if AUTO_BP_UPDATER:
                ...
                elif len(ddb_query_response) > 0 and ddb_bp_count > 1:
        ...
        if OPS_CENTER_INTEGRATION and 'detail' in event:
